...
`start_server()` will pop a Houdini error if the port is already bound. It will **not** print "already running" unless *this* houdini-bin is actually listening. A stale `hou.session.houdinimcp_server` with a dead socket is torn down and rebound.
...
```bash
cd /path/to/houdini-mcp
.venv/bin/python houdini_mcp_server.py
```
...
```bash
HOUDINIMCP_PORT=9877 .venv/bin/python houdini_mcp_server.py
```
...
> Windows support is implemented and unit-tested, but has not been run end to end on a Windows box. Treat the first install as a shakedown and file what breaks.

**`install.py` says "Done!" but the plugin never loads.**
Almost always the preferences directory. Houdini follows the Windows *Personal* known folder, which OneDrive and corporate folder redirection move — so the real prefs can be `C:\Users\you\OneDrive\Documents\houdini22.0` while `%USERPROFILE%\Documents` also exists and is empty. Ask Houdini itself, in its Python Shell:

```python
import hou; print(hou.homeHoudiniDirectory())
```

Then install against that answer:

```
python scripts/install.py --prefs-dir "C:\Users\you\OneDrive\Documents\houdini22.0"
```

`install.py` reads `HOUDINI_USER_PREF_DIR` first (including `__HVER__`), then the registry known folder, then OneDrive, then `~/Documents`. It warns when the directory it picked does not exist yet.

**Port 9876 answers but the commands go nowhere.**
Two Houdini instances. On Win32 `SO_REUSEADDR` means the opposite of the POSIX flag — it lets a second process bind a port already in use. The plugin now uses `SO_EXCLUSIVEADDRUSE`, so the second instance reports the bind failure instead of silently stealing traffic. Check with:

```
netstat -ano | findstr :9876
```

**`capture_desktop` fails.**
It shells out to PowerShell (`System.Drawing.CopyFromScreen`). If PowerShell is not on PATH, or the session is an RDP/service session with no interactive desktop, use `capture_screenshot` instead — that runs inside Houdini and does not need the OS compositor.

**`monitor_render` shows nothing during a Mantra render.**
Fixed: Windows ships Mantra as `mantra.exe`, not the Linux `mantra-bin`. Both are matched now, on the executable name rather than the window title.
...
- If `uv` was installed via PowerShell but isn't found in `cmd.exe`, the PATH update may only apply to PowerShell. Add `%USERPROFILE%\.local\bin` to your system PATH manually. `install.py` now writes the repo's own `.venv\Scripts\python.exe` into `mcp.json` when it exists, so the terminal does not need to resolve `uv` at all.
- The headless hython server is killed with `TerminateProcess` on Windows, so its `SIGTERM` cleanup never runs. Harmless — the bridge cleans up its own side.
