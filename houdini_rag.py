...
PATTERNS_BM25_PATH = Path(os.environ.get(
    "HOUDINIMCP_PATTERNS_BM25", SCRIPT_DIR / "hip_patterns_bm25.json"))
...
class DocumentLoader:
    ...
    def load_all(self):
        documents = []
        for filepath in list(self.docs_dir.rglob('*.md')) + list(self.docs_dir.rglob('*.txt')):
            try:
...
_index = None
_patterns_index = None


def _has_pattern_files(patterns_dir=None):
    d = Path(patterns_dir) if patterns_dir else PATTERNS_DIR
    return d.exists() and any(d.glob("*.txt"))


def get_index():
    """Get or load the global docs index instance.
    ...
    if _index is None:
        ...
        if _index is None:
            ...
            has_patterns = _has_pattern_files()
            if has_docs or has_patterns:
                _index = build_combined_index()
    return _index


def get_patterns_index(patterns_dir=None, cache_path=None):
    """BM25 over hip_patterns/*.txt. Separate from the HFS docs index.

    ingest_hips.py writes gitignored text files; this is how search_docs
    hits them even when houdini_docs_index.json already exists.
    """
    global _patterns_index
    if _patterns_index is not None:
        return _patterns_index
    d = Path(patterns_dir) if patterns_dir else PATTERNS_DIR
    cache = Path(cache_path) if cache_path else PATTERNS_BM25_PATH
    if not _has_pattern_files(d):
        return None
    if cache.exists():
        loaded = BM25Index.load(cache)
        if loaded is not None:
            _patterns_index = loaded
            return _patterns_index
    index = BM25Index()
    for doc in PatternLoader(d).load_all():
        index.add_document(doc["path"], doc["title"], doc["content"])
    index.build()
    try:
        index.save(cache)
...
# A pinned hip_pattern must score at least this fraction of the best doc hit.
PATTERN_PIN_RATIO = 0.5


def search_docs(query, top_k=5):
    """Search Houdini documentation and hip_patterns. Returns list of hits.

    Pattern hits are always surfaced (one slot) when they match — otherwise
    the HFS docs index drowns them.
    """
    index = get_index()
    patterns = get_patterns_index()
    if index is None and patterns is None:
        return {"error": "Docs index not available. Run: python scripts/fetch_houdini_docs.py (and/or scripts/ingest_hips.py)"}
    doc_hits = index.search(query, top_k) if index is not None else []
    pat_hits = patterns.search(query, top_k) if patterns is not None else []
    out = []
    seen = set()
    # Pin the best pattern hit only when it is actually competitive. Pinning
    # unconditionally burned the top slot -- 20% of a default top_k=5 -- on a
    # score-1.9 hit sitting next to score-22 docs, because hip_patterns/ can
    # be a two-file corpus.
    best_doc = doc_hits[0].get("score", 0.0) if doc_hits else 0.0
    # Ratio only -- BM25 scores are not normalised, so an absolute floor
    # would silently drop a pattern that is the single best hit in a small
    # corpus. With no doc hits at all, best_doc is 0 and the pattern pins.
    pinned = bool(pat_hits) and pat_hits[0].get("score", 0.0) >= best_doc * PATTERN_PIN_RATIO
    if pinned:
        first = dict(pat_hits[0])
        first["source"] = "hip_pattern"
        out.append(first)
        seen.add(first.get("path"))
    # When the pin is skipped, pat_hits[0] must still compete for a slot on
    # score. Starting the fill at [1:] regardless discarded the *best*
    # pattern and let a worse one through in its place.
    rest = pat_hits[1:] if pinned else pat_hits
    for hit in list(doc_hits) + list(rest):
        path = hit.get("path")
        if path in seen:
            continue
        rec = dict(hit)
        if path and str(path).startswith("patterns/"):
            rec["source"] = "hip_pattern"
        out.append(rec)
        seen.add(path)
