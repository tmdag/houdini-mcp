...
from houdinimcp.handlers.parameters import (
    ...
    list_file_parms, set_file, is_file_parm,
)
...
class TestParameterHandlers:
    ...
    def test_set_file_picks_file_parm(self):
        fparm = MockParm("fbxfile", "", "FBX File")
        node = MockNodeWithParms("alembic1", "/obj/mcp_geo/alembic1", [self.parm_tx, fparm])
        sys.modules["hou"].node = lambda p: node if p == "/obj/mcp_geo/alembic1" else None
        assert is_file_parm(fparm) is True
        assert is_file_parm(self.parm_tx) is False
        listed = list_file_parms("/obj/mcp_geo/alembic1")
        assert listed["count"] == 1
        assert listed["parms"][0]["name"] == "fbxfile"
        result = set_file("/obj/mcp_geo/alembic1", "/tmp/hero.abc")
        assert result["parm"] == "fbxfile"
        assert result["file"] == "/tmp/hero.abc"
        assert fparm._value == "/tmp/hero.abc"

    def test_set_file_no_file_parm(self):
        with pytest.raises(ValueError, match="No file parm"):
            set_file("/obj/node1", "/tmp/x.abc")
