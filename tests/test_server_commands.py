...
_hou_mock.isUIAvailable = lambda: True
_hou_mock.severityType = types.SimpleNamespace(Error="Error", Fatal="Fatal")
_hou_mock.ui = types.SimpleNamespace(displayMessage=lambda *a, **k: 0, _messages=[])
...
class TestCommandDispatcher:
    ...
    def test_servicing_a_busy_client_does_not_drop_it(self):
        """Ping on B must not wait for capture on A to drop (issue #2).

        The guard moved from conn.busy to the server-wide _executing counter
        when re-entrancy was fixed, but the connection must still survive
        being polled while a command of its own is in flight.
        """
        from houdinimcp.server import _ClientConn
        ...
        a = _ClientConn(_FakeSock(), ("127.0.0.1", 1))
        a.busy = True
        self.server._clients = [a]
        self.server.client = a.sock
        self.server._executing = 1
        try:
            self.server._service_client(a)
        finally:
            self.server._executing = 0
        assert a in self.server._clients
        assert a.sock.closed is False
    ...
    def test_dangerous_code_blocked(self):
        ...
        # Handler stays on the TCP session — error is in the payload.
        assert result["status"] == "success"
        payload = result["result"]
        assert payload["executed"] is False
        assert "Dangerous pattern" in payload["error"]
    ...
    def test_all_handlers_registered(self):
        ...
        expected = [
            ...
            "restart_renderer", "set_hydra_display", "reload_plugin", "set_log_level",
            "frame_selection", "frame_all", "frame_bbox", "set_viewport_direction",
    ...
    def test_bind_handlers_late_imports_viewport(self, monkeypatch):
        import houdinimcp.handlers.viewport as vp
        from houdinimcp.handlers.plugin import bind_handlers

        def fake_list():
            return {"probed": True}

        monkeypatch.setattr(vp, "list_hydra_renderers", fake_list)
        handlers = bind_handlers(self.server)
        assert handlers["list_hydra_renderers"] is fake_list
        assert handlers["reload_plugin"].__name__ == "reload_plugin"

    def test_reload_plugin_does_not_reload_server(self, monkeypatch):
        import importlib
        import houdinimcp.handlers.plugin as plug

        reloaded = []

        def fake_reload(mod):
            reloaded.append(getattr(mod, "__name__", str(mod)))
            return mod

        monkeypatch.setattr(importlib, "reload", fake_reload)
        sys.modules["hou"].session.houdinimcp_server = self.server
        result = plug.reload_plugin()
        assert result["qtimer_untouched"] is True
        assert "houdinimcp.server" not in reloaded
        assert any(n.endswith(".viewport") for n in reloaded)
        assert "reload_plugin" in self.server._get_handlers()
        assert result["patched_instance"] is True

    def test_ensure_reload_hook_patches_missing_key(self, monkeypatch):
        calls = []

        def fake_bootstrap():
            calls.append(True)
            orig = self.server._get_handlers
            self.server._get_handlers = lambda: dict(orig(), reload_plugin=lambda: {})

        monkeypatch.setattr(
            "houdinimcp.handlers.plugin.bootstrap_reload", fake_bootstrap
        )
        self.server._get_handlers = lambda: {"ping": self.server.ping}
        self.server._ensure_reload_hook()
        assert calls == [True]
        assert "reload_plugin" in self.server._get_handlers()

    def test_reload_plugin_command_dispatches(self):
        sys.modules["hou"].session.houdinimcp_server = self.server
        result = self.server.execute_command({"type": "reload_plugin", "params": {}})
        assert result["status"] == "success"
        payload = result["result"]
        assert payload["qtimer_untouched"] is True
        assert payload.get("execute_command_patched") is True
        assert "reload_plugin" in payload["commands"]


class TestBatchIsHonest:
    """batch() groups undo; it does not roll back. Say so, and prove it."""

    def setup_method(self):
        self.server = HoudiniMCPServer.__new__(HoudiniMCPServer)
        self.server.host, self.server.port = "localhost", 9876
        self.server.running, self.server.socket, self.server.client = False, None, None
        self.server.buffer, self.server._clients, self.server.timer = b"", [], None
        from houdinimcp.event_collector import EventCollector
        self.server.event_collector = EventCollector()
        self.applied = []
        ...
        def _ok(name):
            def handler(**params):
                ...
                return {"did": name, "params": params}
            return handler

        def _boom(**params):
            self.applied.append("boom")
            raise RuntimeError("handler exploded")

        self.server._get_handlers = lambda: {
            "op_a": _ok("op_a"), "op_b": _ok("op_b"), "op_boom": _boom,
            "batch": self.server.batch,
        }

    def test_happy_path_runs_every_operation(self):
        result = self.server.batch([
            {"type": "op_a", "params": {"x": 1}},
            {"type": "op_b", "params": {}},
        ])
        assert result["count"] == 2
        assert self.applied == ["op_a", "op_b"]

    def test_unknown_type_is_rejected_before_anything_runs(self):
        """The audit bug: a bad op type left earlier nodes in the scene."""
        with pytest.raises(ValueError) as excinfo:
            self.server.batch([
                {"type": "op_a", "params": {}},
                {"type": "op_b", "params": {}},
                {"type": "nope_not_a_command", "params": {}},
            ])
        assert "Nothing was executed" in str(excinfo.value)
        assert "nope_not_a_command" in str(excinfo.value)
        assert self.applied == [], "pre-validation must run before any handler"

    def test_every_unknown_type_is_listed(self):
        with pytest.raises(ValueError) as excinfo:
            self.server.batch([
                {"type": "bogus_one"}, {"type": "op_a"}, {"type": "bogus_two"},
            ])
        message = str(excinfo.value)
        assert "bogus_one" in message and "bogus_two" in message

    def test_handler_failure_reports_how_far_it_got(self):
        from houdinimcp.server import BatchError
        ...
        err = excinfo.value
        assert err.index == 1
        assert err.cmd_type == "op_boom"
        assert len(err.completed) == 1
        assert err.completed[0]["type"] == "op_a"
        assert "NOT rolled back" in str(err)
        assert self.applied == ["op_a", "boom"], "must stop at the failure"

    def test_batch_error_payload_carries_the_partial_state(self):
        result = self.server.execute_command({
            "type": "batch",
            "params": {"operations": [
                {"type": "op_a", "params": {}},
        ...
        assert result["failed_type"] == "op_boom"
        assert result["rolled_back"] is False

    def test_missing_params_key_is_tolerated(self):
        result = self.server.batch([{"type": "op_a"}])
