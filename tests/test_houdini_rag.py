...
class TestBuildCombinedIndex:
    ...
    def test_search_docs_hits_patterns_even_with_docs_index(self, monkeypatch):
        """Existing houdini_docs_index.json must not hide hip_patterns (#14)."""
        import houdini_rag
        with tempfile.TemporaryDirectory() as tmpdir:
            docs_dir = os.path.join(tmpdir, "docs")
            patterns_dir = os.path.join(tmpdir, "patterns")
            ...
            with open(os.path.join(docs_dir, "sop", "box.md"), "w") as f:
                f.write("= Box SOP =\nCreates a box primitive.")
            with open(os.path.join(patterns_dir, "recipe_unique.txt"), "w") as f:
                f.write("Pattern: UniqueCapybaraJointdeform\nSource: test.hip\njointdeform clip elbowdrop")
            docs_idx = os.path.join(tmpdir, "docs.json")
            pats_bm25 = os.path.join(tmpdir, "pats.json")
            build_index(docs_dir=docs_dir, output_path=docs_idx)
            monkeypatch.setattr(houdini_rag, "_index", None)
            monkeypatch.setattr(houdini_rag, "_patterns_index", None)
            monkeypatch.setattr(houdini_rag, "INDEX_PATH", __import__("pathlib").Path(docs_idx))
            monkeypatch.setattr(houdini_rag, "DOCS_DIR", __import__("pathlib").Path(docs_dir))
            monkeypatch.setattr(houdini_rag, "PATTERNS_DIR", __import__("pathlib").Path(patterns_dir))
            monkeypatch.setattr(houdini_rag, "PATTERNS_BM25_PATH", __import__("pathlib").Path(pats_bm25))
            hits = search_docs("UniqueCapybaraJointdeform", top_k=5)
            assert not isinstance(hits, dict)
            paths = [h["path"] for h in hits]
            assert any(p.startswith("patterns/") for p in paths)


class TestPatternPinDoesNotDropHits:
    """A pattern that loses the pin must still compete for a slot.

    The fill loop started at pat_hits[1:] unconditionally, so when the pin
    was skipped the BEST pattern was discarded and a worse one took its
    place -- with slots to spare.
    """

    def test_unpinned_best_pattern_is_not_discarded(self, monkeypatch):
        import houdini_rag
        ...
        docs = [{"path": "d1", "score": 10.0}, {"path": "d2", "score": 9.0}]
        pats = [{"path": "patterns/best", "score": 4.0},
                {"path": "patterns/worse", "score": 3.0}]
        monkeypatch.setattr(houdini_rag, "get_index", lambda: _Index(docs))
        monkeypatch.setattr(houdini_rag, "get_patterns_index", lambda: _Index(pats))

        hits = houdini_rag.search_docs("q", top_k=5)
        paths = [h["path"] for h in hits]
        # 4.0 < 10.0 * 0.5, so it must not be pinned to slot 0 ...
        assert paths[0] == "d1"
        # ... but it must still be present, and ahead of the worse pattern.
        assert "patterns/best" in paths, paths
        assert paths.index("patterns/best") < paths.index("patterns/worse")

    def test_pinned_pattern_is_not_duplicated(self, monkeypatch):
        import houdini_rag
        ...
        docs = [{"path": "d1", "score": 4.0}]
        pats = [{"path": "patterns/strong", "score": 9.0}]
        monkeypatch.setattr(houdini_rag, "get_index", lambda: _Index(docs))
        monkeypatch.setattr(houdini_rag, "get_patterns_index", lambda: _Index(pats))

        paths = [h["path"] for h in houdini_rag.search_docs("q", top_k=5)]
        assert paths[0] == "patterns/strong"
        assert paths.count("patterns/strong") == 1
