...
def _load_find_render_processes():
    ...
    names_to_extract = {
        "_find_render_processes", "_RENDER_PROCESS_NAMES", "_executable_stem",
    }
    import_whitelist = ("json", "subprocess", "sys", "csv", "io", "os")
    nodes_to_compile = []
    for node in ast.iter_child_nodes(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            if isinstance(node, ast.ImportFrom):
                if node.module and node.module.split(".")[0] in import_whitelist:
                    nodes_to_compile.append(node)
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.split(".")[0] in import_whitelist:
                        nodes_to_compile.append(node)
                        break
...
# Windows ships Mantra as mantra.exe, not mantra-bin.
TASKLIST_MANTRA_WIN = '"mantra.exe","67891","Console","1","80,000 K","Running","USER","0:03:12","N/A"'
# tasklist /V appends the window title, which used to match on substring.
TASKLIST_DECOY = '"notepad.exe","2222","Console","1","10,000 K","Running","USER","0:00:01","husk notes.txt - Notepad"'
...
class TestWindowsProcessNames:
    """Windows ships mantra.exe; matching the whole /V line matched titles."""
    ...
    def _run(monkeypatch, lines):
        ...
        return _find_render_processes()

    def test_windows_mantra_exe_is_detected(self, monkeypatch):
        result = self._run(monkeypatch, [TASKLIST_MANTRA_WIN])
        assert [p["name"] for p in result] == ["mantra"]
        assert result[0]["pid"] == "67891"
        assert result[0]["cpu_time"] == "0:03:12"

    def test_window_title_is_not_a_render(self, monkeypatch):
        """'husk notes.txt - Notepad' is not a Karma render."""
        assert self._run(monkeypatch, [TASKLIST_DECOY]) == []

    def test_decoy_does_not_mask_a_real_render(self, monkeypatch):
        result = self._run(monkeypatch, [TASKLIST_DECOY, TASKLIST_HUSK])
        assert [p["name"] for p in result] == ["husk"]

    def test_csv_header_row_is_ignored(self, monkeypatch):
        result = self._run(monkeypatch, [TASKLIST_HEADER, TASKLIST_UNRELATED])
        assert result == []


class TestExecutableStem:
    """Matching the executable, not the line, is what kills the false positives."""

    stem = staticmethod(_find_render_processes.__globals__["_executable_stem"])

    def test_strips_path_and_arguments(self):
        assert self.stem("/opt/hfs22.0/bin/husk --renderer karma x.usd") == "husk"

    def test_strips_windows_extension(self):
        assert self.stem("husk.exe") == "husk"

    def test_keeps_the_bin_suffix_linux_mantra_uses(self):
        assert self.stem("/opt/hfs22.0/bin/mantra-bin -f driver1") == "mantra-bin"
