...
class TestNodeTypeGlobCaseFolding:
    """Lowercasing a glob corrupts character classes.

    list_node_types used to do fnmatch(name.lower(), pattern.lower()). The
    lowercasing runs inside [] too, so "[A-_]" became the reversed range
    "[a-_]" -- which fnmatch compiles to a never-match -- and "[A-z]"
    stopped matching names starting with an underscore.
    """

    @staticmethod
    def _matches(pattern, name):
        import fnmatch, re
        return bool(re.compile(fnmatch.translate(pattern), re.IGNORECASE).match(name))

    def test_character_class_spanning_underscore(self):
        assert self._matches("[A-_]foo", "_foo")
        assert self._matches("[A-z]*", "_foo")

    def test_still_case_insensitive(self):
        assert self._matches("curvesect", "CurveSect")
        assert self._matches("*INTERSECT*", "curvesect_intersect")

    def test_lowercasing_the_pattern_would_break_these(self):
        import fnmatch
        # the old behaviour, kept as the reason this test exists
        assert not fnmatch.fnmatch("_foo".lower(), "[A-_]foo".lower())
