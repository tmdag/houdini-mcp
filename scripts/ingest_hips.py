...
def _version_key(path):
    """Numeric sort key for a versioned install directory."""
...
def find_houdini_install(hfs_dir=None):
    ...
    if system == "Linux":
        ...
        patterns = ["/Applications/Houdini/Houdini*"]
    elif system == "Windows":
        patterns = [r"C:\Program Files\Side Effects Software\Houdini *"]
    else:
        patterns = []
    # Numeric, not lexical: sorted(reverse=True) on strings ranks hfs9.5
    # above hfs22.0 and "Houdini 9.5" above "Houdini 22.0.429".
...
def discover_hip_files(hfs_path, extra_dirs=None, extra_only=False):
    ...
    extra_only=True skips $HFS help/otls entirely and scans extra dirs only.
...
def cmd_discover(args):
    ...
    extra_only = bool(getattr(args, "extra_only", False))
    files = discover_hip_files(
        hfs_path, extra_dirs=args.extra_dir, extra_only=extra_only)
    if not files:
...
def _add_common_args(parser):
    ...
    parser.add_argument(
        ...
        help="Scan --extra-dir only; do not walk $HFS help/otls",
    )
